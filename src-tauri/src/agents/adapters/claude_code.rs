...
use super::super::{
    install_json_hooks, json_config_has_hoverpet_hook, remove_json_hooks, AdapterError,
...
/// - 改动内容: 在 `hooks` 字段中插入 HoverPet 特定的 JSON 钩子配置。
///   这些钩子会在 Claude Code 触发特定事件（如 `UserPromptSubmit`, `PreToolUse` 等）时，
///   通过命令行调用 HoverPet 的 hook 命令，从而通知桌面宠物更新状态。
