...
fn list_exposes_each_platform_adapter() {
    ...
    let manager = AgentManager::new(temp.path().join(".hoverpet"), temp.path().join("home"));
...
fn adapters_install_repair_and_uninstall_real_config_files() {
    with_opencode_config_dir(|opencode_config_dir| {
        ...
        for adapter_id in ["codex", "claude-code", "gemini", "opencode"] {
            ...
            assert!(
                root.join("hooks/hoverpet-hook.sh").exists(),
...
fn assert_adapter_config_contains_marker(
    ...
    match adapter_id {
        "codex" => {
            ...
            assert!(value.to_string().contains("hoverpet-hook.sh"));
        ...
        "opencode" => {
            let content =
                fs::read_to_string(opencode_config_dir.join("plugins/hoverpet.js")).unwrap();
            assert!(content.contains("hoverpet-managed-hook"));
        }
...
fn assert_adapter_config_does_not_contain_marker(
    ...
    match adapter_id {
        ...
        "opencode" => assert!(!opencode_config_dir.join("plugins/hoverpet.js").exists()),
        _ => unreachable!("unknown adapter"),
    }
}
...
fn assert_json_file_lacks_marker(path: impl AsRef<std::path::Path>) {
    ...
    assert!(!content.contains("hoverpet-hook.sh"));
}
