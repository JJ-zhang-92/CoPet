...
const messages = {
  "en-US": {
    ...
    close: "Close",
    codexTrustDisclosure: "PetHover automatically writes hook trust into ~/.codex/config.toml on install, equivalent to running /hooks → Trust manually in Codex. To revoke, uninstall Codex from PetHover or remove the entries via Codex's /hooks command.",
  ...
  "zh-CN": {
    ...
    backToTop: "返回顶部",
    close: "关闭",
    codexTrustDisclosure: "PetHover 安装时自动向 ~/.codex/config.toml 写入 hook 信任记录，等同于在 Codex 中手动执行 /hooks → 信任。如需撤销，可在 PetHover 中卸载 Codex 集成，或通过 Codex 的 /hooks 命令移除条目。",
    currentPet: "当前宠物",
    customBadge: "自定义",
    english: "English",
