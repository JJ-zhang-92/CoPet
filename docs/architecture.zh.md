...
HoverPet 是基于 Tauri 的桌面电子宠物客户端，面向 Claude Code、Codex、Gemini、OpenCode 等 Agent CLI 工作流。应用首次启动可使用内置宠物；在设置中启用任一 Agent CLI 后，HoverPet 写入对应 CLI 的 hooks，并把 Agent 生命周期事件映射为分层宠物状态。
...
Tauri 应用是唯一长期运行的进程；Agent CLI hooks 只是短生命周期命令，HoverPet 未运行时必须静默退出，不阻塞 Agent 会话。
...
~/.hoverpet/
  ...
  adapters/<id>.json   # HoverPet 已写入的 hook 元数据
```

**内置宠物不写入 `~/.hoverpet/pets`**，而是直接从 bundle 资源 `assets/pets` 解析；删除 `~/.hoverpet` 不会损坏内置宠物。`tauri.conf.json` 的 `assetProtocol.scope` 将 webview 可读路径限制为 `~/.hoverpet/pets`、`~/.codex/pets`、bundle 资源目录。

## 宠物包

HoverPet 使用 Codex 兼容包：`pet.json` + `spritesheet.webp`（或 `.png`），默认 8×9 网格。九行状态：`idle`、`running-right`、`running-left`、`waving`、`jumping`、`failed`、`waiting`、`running`、`review`。
...
Adapter 负责：解析平台相关配置路径、解析已有配置且不丢弃用户设置、只写入带稳定 id 的 HoverPet 条目、修改前备份到 `~/.hoverpet/backups/<adapter-id>/`、记录元数据到 `~/.hoverpet/adapters/<id>.json`。Core 负责：生成 hook command、提供 runtime endpoint/token、事件状态映射、错误本地化。
...
- Hook 命令：超时 500–1000 ms、不等待 UI、HoverPet 未运行时静默退出、避免重量级解释器。
...
- Hooks 配置写入仅允许 adapter 已知路径；写入前读取与解析；格式损坏拒绝覆盖（除非用户明确修复）；首次写入前备份；卸载只移除匹配 HoverPet 元数据的条目；尽量原子写。
- `assetProtocol.scope` 白名单严格限制 webview 可读资源。
- `pet.json` 视为不可信输入；不执行宠物包脚本；图片走浏览器/native 安全解码。

## 国际化

`Locale::EnUs` / `Locale::ZhCn`，用户可显式选或跟随系统。Rust 端的 `MessageKey` 与前端 `src/lib/i18n.ts` 镜像同一组 key；`hoverpet-app-state-changed` 事件触发即时刷新。
...
- Hook 命令打包后优先使用 HoverPet helper 的绝对路径，正确处理空格。
